...
def main1():
	...
	predictor = dlib.shape_predictor(
		p)  # Outputs a set of location points that define a pose of the object. (Here, pose of the human face)
	...
	while (time.time() - currenttime <= 25):  # The calibration code will run for 23 seconds.
		...
		for (i, rect) in enumerate(rects):  # Loop used for the prediction of facial landmarks
			...
			if elapsedTime < 5.0:  # recording the phase where both eyes are open
				...
				cv2.putText(blackimage, 'Open Left eye and close Right Eye', (0, 100), font, 1, (255, 255, 255), 2,
							cv2.LINE_AA)
	...
	leftclick = np.median(
		lclick) - 1  # Calculates mean of the recorded values for left click. Subtracting a constant to make it a bit more flexible
	rightclick = np.median(
		rclick) + 1  # Calculates mean of the recorded values for right click. Adding a constant to make it a bit more flexible
	...
	while (True):
		try:
			...
			for (i, rect) in enumerate(rects):
				...
				[h, k] = shape[33]
				lefteye = EAR(shape[36], shape[37], shape[38], shape[39], shape[40],
							  shape[41])  # Calculation of the EAR for left eye
				righteye = EAR(shape[42], shape[43], shape[44], shape[45], shape[46],
							   shape[47])  # Calculation of the EAR for right eye
				EARdiff = (lefteye - righteye) * 100  # Calculating the difference in EAR in percentage
				mar = MAR(shape[50], shape[58], shape[51], shape[57], shape[52], shape[56], shape[48],
						  shape[54])  # Calculation of the MAR of mouth
				cv2.line(image, (250, 250), (h, k), (0, 0, 0),
						 1)  # Draws a line between the tip of the nose and the centre of reference circle
			...
			if scroll_status == 0:
				if ((h - 250) ** 2 + (k - 250) ** 2 - 50 ** 2 > 0):
					...
					if h > 250:  # The below conditions set the conditions for the mouse to move and that too in any direction we desire it to move to.
						time.sleep(0.03)
						pag.moveTo(pag.position()[0] + (10 * np.cos(1.0 * a)),
								   pag.position()[1] + (10 * np.sin(1.0 * a)), duration=0.01)
			...
			cv2.putText(blackimage, "FPS: " + str(int(1 / (frameTimeFinal - frameTimeInitial))), (0, 150), font, 1,
			...
			blackimage = np.zeros((480, 640, 3), dtype=np.uint8)
			cv2.putText(blackimage, "Landmarks Lost.\nCheck the lighting or reposition your face", (0, 100), font, 1,
						(255, 255, 255), 2, cv2.LINE_AA)
			_, image = cap.read()
			image = cv2.flip(image, 1)
			res = np.vstack((image, blackimage))
